...
Butterfly/FFT dominates the Pareto front at all tested scales ($N{=}4$ to 32): at $N{=}16$ it achieves 83.8\% accuracy at 15$\times$ lower hardware cost than Clements, with 1.6$\times$ better noise robustness.
This advantage grows with insertion loss---at 0.5\,dB/MZI, butterfly drops only 2.3 percentage points while Clements drops 5.4.
...
Nikdast~\textit{et al.}~\cite{nikdast2023} analyze loss and crosstalk for Clements, Reck, and Diamond but do not train networks.
...
A typical value for advanced silicon photonic foundries is $L_{\mathrm{dB}} = 0.2$\,dB~\cite{nikdast2023}.
...
\textbf{Clements}~\cite{clements2016}: a rectangular mesh of alternating even/odd columns of adjacent-pair MZIs. Universal, depth $N$, the standard reference topology.
...
\textbf{SCF Fractal}~\cite{basani2023}: a recursive cosine-sine decomposition. Same MZI count as Clements with depth ${\sim}N$, but error scales theoretically as $O(N\sigma^4)$ rather than $O(N^2\sigma^2)$.
...
Their correct implementation requires non-trivial wiring patterns described in the original papers; they are left for future work. Our open-source framework accepts new topologies as single functions returning layer/port lists, facilitating community contributions.
...
\begin{table}[b]
\caption{Performance at $N{=}16$ on vowel dataset (0.2\,dB/MZI loss). Accuracy at $\sigma{=}0.1$ and robustness ratio (accuracy at $\sigma{=}0.2$ / clean accuracy).}
\label{tab:n16results}
\begin{ruledtabular}
\begin{tabular}{lccccc}
Topology & MZIs & Depth & Cost & Acc.\,(\%) & Robust.\ \\
\hline
Clements    & 120 & 16 & 1920 & 81.8 & 0.209 \\
Reck        & 120 & 29 & 3480 & 84.5 & 0.266 \\
Butterfly   &  32 &  4 &  128 & 83.8 & 0.343 \\
SCF Fractal & 120 & 15 & 1800 & 84.2 & 0.209 \\
\end{tabular}
\end{ruledtabular}
\end{table}

Butterfly occupies the Pareto-optimal lower-left region (low cost, competitive accuracy) at every noise level.
At $N{=}16$ on vowel, butterfly achieves 83.8\% accuracy at hardware cost 128, compared to Clements at 81.8\% and cost 1920---a 15$\times$ cost reduction with comparable accuracy.
On Fashion-MNIST, the pattern is similar: butterfly reaches 79.7\% at cost 128 versus Clements at 78.0\% at cost 1920.
At $N{=}32$ the gap widens further: butterfly achieves 85.5\% at cost 400, versus Clements at 82.2\% at cost 15{,}872 (40$\times$).

Among universal topologies, Reck achieves the highest clean accuracy at $N{=}16$ (84.5\% vowel, 79.0\% fmnist) but at the highest hardware cost (3480).
SCF Fractal matches Clements' accuracy at slightly lower depth but does not reach the Pareto front due to identical MZI count.
...
\caption{Pareto front: accuracy versus hardware cost (MZIs\,$\times$\,depth) for all topologies and mesh sizes. Left column: clean ($\sigma{=}0$). Right column: noisy ($\sigma{=}0.1$\,rad). Top row: vowel dataset. Bottom row: Fashion-MNIST. Dashed line indicates the Pareto front. Butterfly (green triangles) dominates at all scales and noise levels.}
\label{fig:pareto}
\end{figure*}
...
There is a clear monotonic relationship between optical depth and noise sensitivity.
Butterfly (depth~4) retains 34\% of its clean accuracy at $\sigma{=}0.2$ on vowel and 43\% on Fashion-MNIST.
Clements (depth~16) and SCF Fractal (depth~15) retain only 21--25\%.

Reck presents an anomaly: despite having the deepest topology (depth~29), it achieves robustness of 0.266 on vowel---better than Clements (0.209).
We attribute this to Reck's triangular structure, in which some output ports are reached through fewer MZIs than the maximum depth, effectively creating a distribution of path lengths rather than a uniform depth.
This deserves further investigation.

SCF Fractal's theoretical $O(N\sigma^4)$ error scaling advantage~\cite{basani2023} is not apparent in our results.
At $N{=}16$, SCF (depth~15) and Clements (depth~16) show essentially identical robustness (0.209 vs.\ 0.209).
...
Table~\ref{tab:loss} shows the effect of increasing per-MZI insertion loss from 0.2 to 0.5\,dB at $N{=}16$ on vowel.
Butterfly is the most loss-tolerant topology: its clean accuracy drops only 2.3 percentage points (pp) from 0.2 to 0.5\,dB/MZI, because signals traverse only 4 MZIs.
Clements drops 5.4\,pp and SCF Fractal drops 5.7\,pp, as their signals traverse 15--16 MZIs.
...
\begin{table}[t]
\caption{Insertion loss sensitivity at $N{=}16$, vowel dataset. Clean accuracy (\%) and robustness ratio at three loss levels.}
\label{tab:loss}
\begin{ruledtabular}
...
\hline
Clements    & 81.8 & 0.21 & 84.5 & 0.21 & 76.4 & 0.22 \\
Reck        & 84.5 & 0.27 & 77.8 & 0.29 & 81.5 & 0.32 \\
Butterfly   & 83.8 & 0.34 & 85.5 & 0.36 & 81.5 & 0.38 \\
SCF Fractal & 84.2 & 0.21 & 82.5 & 0.21 & 78.5 & 0.22 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Table~\ref{tab:noiseaware} compares standard and noise-aware training for Clements and SCF Fractal at $N{=}16$ on vowel.
Noise-aware training (injecting $\sigma{=}0.05$ during training) improves robustness ratios: +34\% relative improvement for Clements, +26\% for SCF Fractal.
However, this comes at a steep cost in clean accuracy: $-$19.5\,pp for Clements and $-$12.5\,pp for SCF Fractal.

Critically, butterfly \emph{without} noise-aware training achieves better robustness (0.343) than Clements \emph{with} it (0.281).
This demonstrates that \textbf{topology selection is more effective than training strategy for achieving noise robustness}.

\begin{table}[t]
\caption{Standard versus noise-aware training at $N{=}16$, vowel, 0.2\,dB/MZI. Noise-aware training injects $\sigma{=}0.05$ during training.}
...
\hline
Clements    & 81.8 & 0.209 & 62.3 & 0.281 \\
SCF Fractal & 84.2 & 0.209 & 71.7 & 0.263 \\
\hline
Butterfly (std) & 83.8 & 0.343 & \multicolumn{2}{c}{---} \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\caption{Clean accuracy versus mesh size $N$ on vowel. Butterfly matches or exceeds universal topologies at all tested sizes despite its non-universality. At $\sigma{=}0.1$ (not shown), the gap widens further in butterfly's favor.}
\label{fig:scaling}
\end{figure}
...
\textit{The benchmark as a contribution.}
...
We encourage the community to contribute validated implementations of diamond, braid, and other emerging topologies to enable a more comprehensive comparison.
...
We presented the first unified Pareto comparison of four MZI mesh topologies---Clements, Reck, butterfly/FFT, and SCF fractal---under identical training, insertion loss, and phase noise conditions.
Across 36 experiments spanning two datasets, four mesh sizes, three insertion loss levels, and noise-aware training, butterfly/FFT consistently dominates the Pareto front.
At $N{=}16$, it achieves comparable accuracy to universal topologies at 15$\times$ lower hardware cost with 1.6$\times$ better noise robustness.
This advantage is driven by its $O(\log N)$ optical depth and amplified by insertion loss---it grows with both mesh size and foundry impairments.

Training-time noise injection improves deep topologies but cannot compensate for the depth disadvantage: butterfly without noise-aware training outperforms Clements with it.
Topology selection is more effective than training strategy for achieving robustness.
...
\bibitem{tian2022}
H.~Tian, L.~Zhao, X.~Li, and W.~Bogaerts,
\textit{FFTUnitary: an energy-efficient non-universal photonic neural network architecture},
Nanophotonics \textbf{11}, 4867 (2022).

\bibitem{feng2022}
C.~Feng, Z.~Ying, Z.~Zhao, J.~Gu, D.~Z.~Pan, and R.~T.~Chen,
\textit{Compact design of a programmable photonic neural network based on butterfly-style MZI mesh},
ACS Photonics \textbf{9}, 3906 (2022).

\bibitem{basani2023}
S.~R.~Basani and A.~V.~Velasco,
\textit{Self-similar cosine-sine fractal architecture for multiport photonic processors},
Nanophotonics \textbf{12}, 5 (2023).
...
T.~Marchesin, E.~Diamanti, and N.~Treps,
\textit{Braided multiport interferometers with balanced optical paths},
Opt.\ Express \textbf{33}, 4227 (2025).
...
\bibitem{nikdast2023}
M.~Nikdast, G.~Pasricha, and S.~Abadal,
...
\bibitem{pai2023}
S.~Pai, Z.~Sun, T.~W.~Hughes, T.~Park, B.~Bartlett, I.~A.~D.~Williamson, M.~Minkov, M.~Milanovi\'{c}, N.~Joshi, O.~Moreira, O.~Reshef, and D.~A.~B.~Miller,
...
\textit{Optimal design of the universal multiport interferometer with minimal optical depth},
Opt.\ Express \textbf{32}, 11004 (2024).
