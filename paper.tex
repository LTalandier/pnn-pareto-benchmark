...
At $N{=}16$ on vowel, butterfly achieves $84.3 \pm 1.7$\% accuracy at hardware cost 128, compared to Clements at $82.7 \pm 0.9$\% and cost 1920---a 15$\times$ cost reduction with comparable accuracy.
...
Standard training results are averaged over 4 random seeds; noise-aware results are single runs.
Noise-aware training (injecting $\sigma{=}0.05$ during training) improves robustness ratios: +35\% relative for Clements (from $0.208$ to $0.281$), +25\% for SCF Fractal (from $0.210$ to $0.263$).
However, this comes at a steep cost in clean accuracy: $-$20.4\,pp for Clements (from $82.7 \pm 0.9$\% to 62.3\%) and $-$13.0\,pp for SCF Fractal (from $84.7 \pm 0.9$\% to 71.7\%).

Critically, butterfly \emph{without} noise-aware training achieves better robustness ($0.377 \pm 0.032$) than Clements \emph{with} it (0.281).
This demonstrates that \textbf{topology selection is more effective than training strategy for achieving noise robustness}.

\begin{table}[t]
\caption{Standard versus noise-aware training at $N{=}16$, vowel, 0.2\,dB/MZI. Standard training values are mean $\pm$ std over 4 seeds. Noise-aware training injects $\sigma{=}0.05$ during training (single run).}
...
\hline
Clements    & $82.7 \pm 0.9$ & $0.208$ & 62.3 & 0.281 \\
SCF Fractal & $84.7 \pm 0.9$ & $0.210$ & 71.7 & 0.263 \\
\hline
Butterfly (std) & $84.3 \pm 1.7$ & $0.377$ & \multicolumn{2}{c}{---} \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Across 72 experiments spanning two datasets, four mesh sizes, three insertion loss levels, and noise-aware training, butterfly/FFT consistently dominates the Pareto front.
At $N{=}16$, it achieves comparable accuracy to universal topologies at 15$\times$ lower hardware cost with 1.8$\times$ better noise robustness.
