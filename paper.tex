...
This advantage grows with insertion loss: at 0.5\,dB/MZI, butterfly drops only 2.6 percentage points (averaged over 4 seeds) while Clements drops 4.6.
...
Photonic neural networks based on meshes of Mach--Zehnder interferometers implement matrix--vector multiplication in the optical domain, offering potential advantages in throughput and energy efficiency over electronic accelerators~\cite{shen2017,tsakyridis2024,bandyopadhyay2024}.
...
Shafiee~\textit{et al.}~\cite{shafiee2024} analyze loss and crosstalk for Clements, Reck, and Diamond but do not train networks.
...
A typical value for advanced silicon photonic foundries is $L_{\mathrm{dB}} = 0.2$\,dB~\cite{shafiee2024}.
...
At 30 trials, the standard error of the mean accuracy is below 0.015 for all configurations tested, confirming adequate convergence.
...
PCA at $N{=}4, 8, 16$ retains 68.9\%, 91.2\%, and 100\% of the variance, respectively; the dataset is fully represented at $N{=}16$.
...
PCA at $N{=}4, 8, 16$ retains only 47.0\%, 59.2\%, and 67.6\% of the variance, so accuracy is partly limited by the input representation rather than the mesh topology.
...
\caption{Pareto front: accuracy versus hardware cost (MZIs\,$\times$\,depth) for all topologies and mesh sizes. Left column: clean ($\sigma{=}0$). Right column: noisy ($\sigma{=}0.1$\,rad). Top row: vowel dataset (mean $\pm$ std over 4 seeds). Bottom row: Fashion-MNIST (mean $\pm$ std over 3--4 seeds). Dashed line indicates the Pareto front computed from mean values.}
\label{fig:pareto}
\end{figure*}
...
However, butterfly implements only a strict subgroup of $U(N)$: for applications requiring arbitrary unitary transformations (quantum computing, optical channel estimation), universal topologies remain necessary.
Reduced-depth universal architectures~\cite{tang2024} offer an intermediate point between Clements and butterfly on the depth--universality tradeoff and merit future benchmarking.
...
We also tested 2-layer architectures with modReLU nonlinearity at $N{=}16$ on vowel.
Adding a second layer did not improve accuracy for either topology: butterfly dropped from $84.3 \pm 1.7$\% to $79.6 \pm 3.2$\%, while Clements collapsed to $45.8 \pm 15.3$\% with high variance across seeds.
The doubled optical depth (8 and 32 layers respectively) appears to outweigh any expressiveness gain, suggesting that for these tasks and noise levels, a single shallow layer is preferable.

\textit{What we did not model.}
Our simulations have several simplifications that could affect the relative topology ranking:
\begin{itemize}
\item \emph{Waveguide crossing loss.} Butterfly requires physical waveguide crossings for non-adjacent port connections (visible as dashed lines in Fig.~\ref{fig:schematics}c,d). At ${\sim}0.02$\,dB per crossing~\cite{shafiee2024}, this could partially offset butterfly's depth advantage. As a rough estimate, at $N{=}16$ each butterfly stage has stride-$2^k$ connections requiring $O(N)$ crossings across $\log_2 N = 4$ stages, totaling ${\sim}30$ crossings per path and adding ${\sim}0.6$\,dB of loss. This is comparable to one additional MZI and would reduce but not eliminate butterfly's advantage over Clements (whose planar layout requires no crossings). A precise estimate requires layout-level analysis specific to the foundry.
...
\item \emph{Fault-tolerant MZI designs.} Hamerly~\textit{et al.}~\cite{hamerly2022nat} demonstrated 3-splitter MZI architectures where matrix fidelity does not degrade with mesh size. Such building blocks could fundamentally alter the depth--robustness tradeoffs reported here, and their interaction with different mesh topologies is an important open question.
\end{itemize}
...
Table~\ref{tab:guidelines} summarizes these recommendations.

\begin{table}[t]
\caption{Topology selection guidelines based on this study's findings.}
\label{tab:guidelines}
\begin{ruledtabular}
\begin{tabular}{lll}
Use case & Recommendation & Rationale \\
\hline
Classification / inference & Butterfly & 15$\times$ lower cost, best \\
 & & robustness, non-universality \\
 & & not limiting \\
Lossy foundry & Butterfly & Depth advantage amplified \\
($>$\,0.3\,dB/MZI) & & by insertion loss \\
Universality required & Clements & Full $U(N)$ coverage at \\
(quantum, channel est.) & & lower depth than Reck \\
Improving robustness & Change topology & Topology choice more \\
of deep meshes & first & effective than noise-aware \\
 & & training \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
We compared four MZI mesh topologies under identical training, insertion loss, and phase noise conditions across 145 experiments.
