...
\caption{Pareto front: accuracy versus hardware cost (MZIs\,$\times$\,depth) for all topologies and mesh sizes. Left column: clean ($\sigma{=}0$). Right column: noisy ($\sigma{=}0.1$\,rad). Top row: vowel dataset (mean $\pm$ std over 4 seeds). Bottom row: Fashion-MNIST (mean $\pm$ std over 3--4 seeds at $N{=}4,8$; single run at $N{=}16$). Dashed line indicates the Pareto front computed from mean values.}
\label{fig:pareto}
\end{figure*}
...
We compared four MZI mesh topologies under identical training, insertion loss, and phase noise conditions across 133 experiments.
